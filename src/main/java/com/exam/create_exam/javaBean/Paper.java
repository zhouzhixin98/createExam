package com.exam.create_exam.javaBean;
...
//    private List<QuestionBean> questionList = new ArrayList<QuestionBean>();
    private List<AeTopic> questionList = new ArrayList<AeTopic>();
    ...
    public double getTotalScore() {
        if (totalScore == 0) {
            double total = 0;
            for (AeTopic question : questionList) {
    ...
    public double getDifficulty() {
        if (difficulty == 0) {
            ...
            for (AeTopic question : questionList) {
                _difficulty += question.getScore() * question.getComplexity();
            }
    ...
    public void setKpCoverage(RuleBean rule) {
        if (kPCoverage == 0) {
            ...
            Set<String> another = questionList.stream().map(questionBean -> String.valueOf(questionBean.getKpId())).collect(Collectors.toSet());
            // 交集操作
    ...
    public boolean containsQuestion(AeTopic question) {
        if (question == null) {
            ...
            for (AeTopic aQuestionList : questionList) {
     ...
     * @param question
     */
    public void saveQuestion(int index, AeTopic question) {
    ...
    public void addQuestion(AeTopic question) {
    ...
    public AeTopic getQuestion(int index) {
    ...
    public List<AeTopic> getQuestionList() {
        return questionList;
    }

    public void setQuestionList(List<AeTopic> questionList) {
